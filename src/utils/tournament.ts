...
export const getProposedSwissPairings = (tournament: Tournament): { player1Id: number, player2Id: number, round: number }[] => {
  ...
  // Simple rule: Wait for all matches in current round to complete
  if (playersWaitingForPairing.length > 0) {
    console.log(`Waiting for ${playersWaitingForPairing.length} players to finish their matches`);
    return []; // No rapid pairings - wait for round to complete
  }
