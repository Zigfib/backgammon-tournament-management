...
const SwissDashboard: React.FC<SwissDashboardProps> = ({ tournament, setTournament }) => {
  ...
  const availablePlayers = playerStatuses.filter(ps => ps.status === 'available');
  const finishedPlayers = playerStatuses.filter(ps => ps.status === 'finished');
    ...
    <div className="swiss-dashboard">
      <div style={{ marginBottom: '20px' }}>
        ...
        <div style={{ marginBottom: '15px', padding: '10px', backgroundColor: '#f5f5f5', borderRadius: '5px' }}>
          <strong>Round {currentRound} of {tournament.numRounds}</strong>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '20px' }}>
        ...
        <div className="status-section">
          ...
          <div style={{ backgroundColor: '#fff5f5', padding: '15px', borderRadius: '5px', border: '1px solid #ffcccb' }}>
            {playingPlayers.length === 0 ? (
              <p style={{ color: '#666', fontStyle: 'italic' }}>No matches in progress</p>
            ) : (
              playingPlayers.map(({ player, currentMatch }) => (
                <div key={player.id} style={{ marginBottom: '10px', padding: '8px', backgroundColor: 'white', borderRadius: '3px' }}>
                  <strong>{player.name}</strong> ({player.points} pts)
                  {currentMatch && (
                    <div style={{ fontSize: '0.9em', color: '#666' }}>
                      vs {tournament.players.find(p => 
                        p.id === (currentMatch.player1 === player.id ? currentMatch.player2 : currentMatch.player1)
                      )?.name} - Round {currentMatch.round}
                    </div>
                  )}
                </div>
              ))
        ...
        <div className="status-section">
          ...
          <div style={{ backgroundColor: '#f5fff5', padding: '15px', borderRadius: '5px', border: '1px solid #ccffcc' }}>
            {availablePlayers.length === 0 ? (
              <p style={{ color: '#666', fontStyle: 'italic' }}>No players waiting</p>
            ) : (
              <div>
                {availablePlayers.map(({ player }) => (
                  <div key={player.id} style={{ marginBottom: '8px', padding: '8px', backgroundColor: 'white', borderRadius: '3px' }}>
                    <strong>{player.name}</strong> ({player.points} pts)
                    <div style={{ fontSize: '0.9em', color: '#666' }}>
                ...
                {availablePlayers.length >= 2 && (
                  <div style={{ marginTop: '15px', padding: '10px', backgroundColor: '#e8f5e8', borderRadius: '3px' }}>
                        ...
                        return Object.entries(byPoints)
                          ...
                          .map(([points, players]) => (
                            <div key={points} style={{ marginBottom: '5px' }}>
                              <span style={{ fontWeight: 'bold' }}>{points} pts:</span> {players.map(p => p.name).join(' vs ')}
                            </div>
                          ));
                      })()}
                    </div>
            ...
            <div style={{ backgroundColor: '#f9f9f9', padding: '15px', borderRadius: '5px', border: '1px solid #ddd' }}>
              {finishedPlayers.map(({ player }) => (
                <div key={player.id} style={{ marginBottom: '8px', padding: '8px', backgroundColor: 'white', borderRadius: '3px' }}>
                  <strong>{player.name}</strong> ({player.points} pts)
                  <div style={{ fontSize: '0.9em', color: '#666' }}>
                    Final ELO: {player.currentElo} ({player.currentElo >= player.startingElo ? '+' : ''}{player.currentElo - player.startingElo})
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* Tournament Progress */}
      <div style={{ marginTop: '20px', padding: '15px', backgroundColor: '#f0f8ff', borderRadius: '5px', border: '1px solid #add8e6' }}>
        <h4 style={{ margin: '0 0 10px 0' }}>Tournament Progress</h4>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '10px' }}>
          <div>
            <strong>Completed Matches:</strong><br />
            {tournament.matches.filter(m => m.completed).length} / {tournament.matches.length}
          </div>
          <div>
            <strong>Players Finished:</strong><br />
            {finishedPlayers.length} / {tournament.players.length}
          </div>
          <div>
            <strong>Average Points:</strong><br />
            {(tournament.players.reduce((sum, p) => sum + p.points, 0) / tournament.players.length).toFixed(1)}
          </div>
        </div>
      </div>
    </div>
